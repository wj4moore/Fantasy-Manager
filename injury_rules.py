# injury_rules.py
from __future__ import annotations
import pandas as pd

# Default multipliers for common statuses
DEFAULT_STATUS_MULTIPLIER = {
    # Game-level statuses
    "IR": 0.00,
    "OUT": 0.00,
    "SUSPENDED": 0.00,
    "DOUBTFUL": 0.10,
    "QUESTIONABLE": 0.85,
    "PROBABLE": 0.95,   # legacy, rarely used
    "ACTIVE": 1.00,
    "NA": 1.00,
    None: 1.00,
}

# Practice trend: a small signal if present
DEFAULT_PRACTICE_MULTIPLIER = {
    "DNP": 0.85,
    "LP": 0.95,   # Limited Participant
    "FP": 1.00,   # Full Participant
    None: 1.00,
}

def _norm(s):
    if s is None:
        return None
    return str(s).strip().upper()

def infer_status_from_players_map(players_map: dict, player_id: str) -> dict:
    """
    Pull injury hints from Sleeper players map (if present). We keep this generic:
      - injury_status (e.g., 'OUT', 'Q', 'D', 'IR')
      - injury_body_part (unused for now)
      - status (sometimes 'Active', 'Inactive')
    Returns a dict with 'game_status' and 'practice_status' best-effort normalized.
    """
    meta = players_map.get(player_id, {}) if isinstance(players_map, dict) else {}
    inj = _norm(meta.get("injury_status"))
    # normalize common short tags
    short = {"Q": "QUESTIONABLE", "D": "DOUBTFUL", "O": "OUT"}
    inj = short.get(inj, inj)

    # Sleeper sometimes has 'status': 'Active'/'Inactive'
    status = _norm(meta.get("status"))
    if status == "INACTIVE" and inj is None:
        inj = "OUT"
    if status == "ACTIVE" and inj is None:
        inj = "ACTIVE"

    return {
        "game_status": inj,
        "practice_status": None,  # not in players map typically
    }

def load_injury_csv(path: str) -> pd.DataFrame:
    """
    Optional CSV to override/augment injuries.
    Expected columns (case-insensitive ok):
      player_id, game_status, practice_status
    Extra columns are ignored.
    """
    df = pd.read_csv(path)
    cols = {c.lower(): c for c in df.columns}
    need = {"player_id"}
    if not need.issubset(set(cols.keys())):
        raise ValueError("injury CSV must include at least 'player_id' column")

    # Normalize columns
    out = pd.DataFrame({
        "player_id": df[cols["player_id"]].astype(str)
    })
    out["game_status"] = df[cols.get("game_status", None)].map(_norm) if "game_status" in cols else None
    out["practice_status"] = df[cols.get("practice_status", None)].map(_norm) if "practice_status" in cols else None
    return out

def apply_injury_adjustments(
    df_proj: pd.DataFrame,
    players_map: dict | None = None,
    injury_csv: str | None = None,
    status_multiplier: dict | None = None,
    practice_multiplier: dict | None = None,
    verbose: bool = True,
) -> pd.DataFrame:
    """
    Input df_proj must include: ['player_id','position','projected'] (projected = current projected mean).
    Returns a copy with columns:
      - injury_game_status
      - injury_practice_status
      - injury_mult
      - projected_adj  (projected * injury_mult)
    Order of precedence: CSV overrides players_map; if neither has info → assume ACTIVE (1.0).
    """
    if status_multiplier is None:
        status_multiplier = DEFAULT_STATUS_MULTIPLIER
    if practice_multiplier is None:
        practice_multiplier = DEFAULT_PRACTICE_MULTIPLIER

    df = df_proj.copy()
    df["injury_game_status"] = None
    df["injury_practice_status"] = None

    # 1) seed from players_map
    if players_map:
        seeds = df["player_id"].apply(lambda pid: infer_status_from_players_map(players_map, pid))
        df["injury_game_status"] = seeds.map(lambda d: d.get("game_status"))
        df["injury_practice_status"] = seeds.map(lambda d: d.get("practice_status"))

    # 2) CSV overrides if provided
    if injury_csv:
        try:
            inj = load_injury_csv(injury_csv)
            df = df.merge(inj, on="player_id", how="left", suffixes=("", "_csv"))
            # Prefer CSV values when present
            df["injury_game_status"] = df["game_status"].combine_first(df["injury_game_status"])
            df["injury_practice_status"] = df["practice_status"].combine_first(df["injury_practice_status"])
            df.drop(columns=[c for c in ["game_status", "practice_status"] if c in df.columns], inplace=True)
        except Exception as e:
            if verbose:
                print("[injury] CSV load failed; using players_map only:", e)

    # Normalize and map multipliers
    df["injury_game_status"] = df["injury_game_status"].map(_norm)
    df["injury_practice_status"] = df["injury_practice_status"].map(_norm)

    df["injury_mult"] = df["injury_game_status"].map(status_multiplier).fillna(1.0)
    df["injury_mult"] *= df["injury_practice_status"].map(practice_multiplier).fillna(1.0)

    # Never boost above 1.0 via practice; cap in [0,1]
    df["injury_mult"] = df["injury_mult"].clip(lower=0.0, upper=1.0)

    # Apply to projections (DEF/K stay as-is unless explicitly OUT)
    df["projected_adj"] = df["projected"]
    mask_players = df["position"].isin(["QB", "RB", "WR", "TE"])
    df.loc[mask_players, "projected_adj"] = df.loc[mask_players, "projected"] * df.loc[mask_players, "injury_mult"]

    # Friendly log
    if verbose:
        flagged = df[(df["injury_mult"] < 1.0) & mask_players][["name", "position", "projected", "injury_game_status", "injury_practice_status", "injury_mult"]]
        if not flagged.empty:
            print("\n[injury] Adjusted projections based on status/practice:")
            print(flagged.sort_values("injury_mult").to_string(index=False))

    return df
