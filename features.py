...
# Imports: core libs and local pipeline.
import pandas as pd
import numpy as np
from pathlib import Path
from data_pipeline import load_pbp

# Schema: columns the model expects from offline features.
FEATURE_COLS = [
...
# Utils: safe integer conversion (NaNs→0 then astype(int)).
...
# Utils: safe float conversion (coerce then fill 0.0).
def _as_float(s: pd.Series) -> pd.Series:
    return pd.to_numeric(s, errors="coerce").fillna(0.0)

# Utils: pick the first available column from candidates.
def _pick_col(df: pd.DataFrame, candidates: list[str], default=0):
    for c in candidates:
        if c in df.columns:
            return df[c]
    return pd.Series(default, index=df.index) if isinstance(default, (int, float)) else default

# Scoring: simple half-PPR-ish fantasy points.
def compute_fantasy_points(agg: pd.DataFrame) -> pd.Series:
    return (
        0.04 * _as_float(agg.get("passing_yards", 0))
        + 4.00 * _as_float(agg.get("passing_tds", 0))
        - 1.00 * _as_float(agg.get("interceptions", 0))
        + 0.10 * _as_float(agg.get("rushing_yards", 0))
        + 6.00 * _as_float(agg.get("rushing_tds", 0))
        + 0.10 * _as_float(agg.get("receiving_yards", 0))
        + 6.00 * _as_float(agg.get("receiving_tds", 0))
        + 0.50 * _as_float(agg.get("receptions", 0))
    )

# Builder: per-player-week aggregates + simple rolling features.
def build_player_week_features(years, out_csv: str = "data/features_player_week.csv") -> str:
    # Load: play-by-play for seasons.
    pbp = load_pbp(years)

    # IDs: derive a best-effort player id per play.
    ...
    pbp = pbp.assign(any_player_id=any_player_id).dropna(subset=["any_player_id"])

    # Game Meta: normalize common team/week/season fields.
    ...
    # Stats: yards/TDs/peripherals across schemas.
    ...
    # Receptions: prefer 'reception'; fallback to 0 if absent.
    receptions = _as_int(_pick_col(pbp, ["reception", "receptions"], default=0))

    # Frame: per-play to per-player-week base table.
    df = pd.DataFrame({
    ...
    # Aggregate: sum stats by player-week-team context.
    ...
    # Fantasy: compute fantasy points.
    agg["fantasy_points"] = compute_fantasy_points(agg)

    # Home/Away: binary home flag from posteam vs home_team.
    agg["is_home"] = (agg["posteam"] == agg["home_team"]).astype(float)

    # Defense Rank: season-average FP allowed by defense (proxy pass & rush).
    def_allowed = (
        agg.groupby(["defteam", "season"], dropna=False)["fantasy_points"]
           .mean()
           .reset_index(name="fp_allowed")
    )
    def_allowed["opp_rank"] = def_allowed.groupby("season")["fp_allowed"].rank(ascending=True)

    agg = agg.merge(
        def_allowed[["defteam", "season", "opp_rank"]]
                  .rename(columns={"opp_rank": "opp_pass_rank"}),
        on=["defteam", "season"],
        how="left",
    )
    agg["opp_rush_rank"] = agg["opp_pass_rank"]

    # Rolling: 3-week rolling FP & “opportunity” proxy.
    agg = agg.sort_values(["any_player_id", "season", "week"])
    agg["rolling_fp_3"] = (
        agg.groupby(["any_player_id", "season"])["fantasy_points"]
           .rolling(3, min_periods=1).mean()
           .reset_index(level=[0, 1], drop=True)
    )

    # Opportunity: receptions + scaled rush yards.
    agg["oppty"] = agg["receptions"].fillna(0) + agg["rushing_yards"].fillna(0) / 5.0
    agg["rolling_oppty_3"] = (
        agg.groupby(["any_player_id", "season"])["oppty"]
           .rolling(3, min_periods=1).mean()
           .reset_index(level=[0, 1], drop=True)
    )

    # Placeholders: vegas & weather constants for offline features.
    agg["vegas_total"]  = 45.0
    agg["weather_wind"] = 5.0

    # Output: compact training frame with labels.
    out = agg[[
        "any_player_id", "season", "week", "posteam", "defteam",
        "is_home", "opp_pass_rank", "opp_rush_rank",
        "rolling_fp_3", "rolling_oppty_3",
        "vegas_total", "weather_wind",
        "fantasy_points",
    ]].rename(columns={"any_player_id": "player_id"})

    # Save: write CSV for training pipeline.
